...
| Dan 12 | RAG minimalni demo — chunking + embedding + retrieval nad malim sintetičkim policy korpusom (EN + AR), sa citatom uz svaki vraćeni pasus (SPEC.md sekcija 9) | ✅ **Implementirano** (2026-09-10) — `sample_docs/policy_corpus/` (8 fiktivnih "SCB policy" .md fajlova, 4 EN + 4 AR: SME tenor, LTV, DSCR pragovi, zabranjeni sektori, watch-list/restruktuiranje). `src/rag/` — `chunk.py` (fiksni prozor + overlap, odvojeno EN 450/90 i AR 350/70), `corpus.py`, `index.py` (in-memory numpy kosinus nad `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`, CPU, ~1s indeks), `retrieve.py` (čist contract: query → `[{text, doc_id, chunk_id, lang, score}]`). **NIJE peti čvor u ingest→extract→validate** — izolovan; u portalu je zaseban sidebar expander "Policy lookup (RAG demo)" koji ne dira polja ni memo. Gold set: `evaluation/rag_gold.py` (13 pitanja, 7 EN + 6 AR), sweep `evaluation/run_rag_eval.py` → **13/13 rank-1 tačan, 13/13 u top-3**. 25 novih testova. NIJE proposal §9.3 KPI (korpus namerno premali). **Spojeno sa portalom (read-only):** (1) sidebar "Policy lookup (RAG demo)" — slobodan upit; (2) po dokumentu, expander "Policy check (RAG)" — `src/rag/policy_check.py` pravi policy pitanja IZ izvučenih polja (traženi iznos, kolateral, postojeći facility-ji, delatnost) i vraća relevantne pasuse sa citatom. NE menja polja ni memo, nije compliance odluka — to bi bio Policy Monitor (Risk Agent), i dalje van POC obima. |
